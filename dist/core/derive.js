 ...
 */
function extractAssignee(lines) {
 ...
 */
function extractEstimatedEffort(lines) {
 ...
 */
export function checkAndInvalidateDrift(planDir, currentFingerprint) {
    const planYamlPath = join(planDir, 'plan.yaml');
    if (!existsSync(planYamlPath))
        return { type: 'no-existing-yaml' };
    ...
    try {
        ...
        existingFingerprint = parsed?.source_md_fingerprint ?? null;
    }
    catch (e) {
...
export async function derivePlan(projectRoot, planId) {
    ...
    if (parsed.tags)
        yamlContent.tags = parsed.tags;
    if (parsed.assignee)
        yamlContent.assignee = parsed.assignee;
    if (parsed.dependencies)
        yamlContent.dependencies = parsed.dependencies;
    if (parsed.estimated_effort)
        yamlContent.estimated_effort = parsed.estimated_effort;
