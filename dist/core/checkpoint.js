...
export async function createCheckpoint(projectRoot, planId, reason) {
    ...
    if (!existsSync(reviewPath))
        throw new Error(`review-report.md not found`);
    ...
    const storedFp = yamlParsed?.source_md_fingerprint;
    if (!storedFp) {
        throw new Error(`plan.yaml missing source_md_fingerprint; re-run \`plan derive\`.`);
    }
    if (storedFp !== currentFp) {
        throw new Error(`fingerprint mismatch: plan.md (${currentFp}) does not match plan.yaml source fingerprint (${storedFp}). Re-run \`plan derive\`.`);
    }
    const reviewContent = readFileSync(reviewPath, 'utf-8');
    // Read validation report
    const validationPath = join(planDir, 'validation-report.yaml');
    if (!existsSync(validationPath))
        throw new Error(`validation-report.yaml not found`);
